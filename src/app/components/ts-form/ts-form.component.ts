...
export class TsFormComponent implements OnInit {
  ...
  constructor(private adminService: AdminService, private authService: AuthenticationService,
    ...
    @Inject(MAT_DIALOG_DATA) public data: Data, private router: Router, private activatedRoute: ActivatedRoute, ) { }
  ...
  onFileChange(ev) {
    ...
    reader.onload = (event) => {
      ...
      jsonData = workBook.SheetNames.reduce((initial, name) => {
        ...
        // this.data1 = <AOA>(XLSX.utils.sheet_to_json(sheet, { header: 1 }));
      ...
      for (var key in req_data[mainKey]) {
        ...
        for (var i = 0; i < this.pros.length; i++) {
          if (req_data[mainKey][key]['ProjectName'].replace(/\s/g, "") == this.pros[i]['value'].replace(/\s/g, "")) {
            ...
            // reqData.push(req_date + " " + req_data[mainKey][key]['Start'] + ":00")
            var st = new Date(req_date + " " + req_data[mainKey][key]['Start'] + ":00")
            // console.log("st",st)
            reqData.push(st)
            // reqData.push(req_date + " " + req_data[mainKey][key]['End'] + ":00")
            // reqData.push(new Date((req_date1 + " " + req_data[mainKey][key]['End'] + ":00")))
            var end = new Date(req_date + " " + req_data[mainKey][key]['End'] + ":00")
            // console.log("end",end)
            reqData.push(end)
